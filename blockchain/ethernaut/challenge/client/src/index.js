...
async function switchToCustomNetwork() {
  try {
    console.info(`${constants.ACTIVE_NETWORK.url}:${constants.ACTIVE_NETWORK.port}`);
      ...
      params: [
        {
          chainId: '0x53a',
          rpcUrls: [
            `${constants.ACTIVE_NETWORK.url}:${constants.ACTIVE_NETWORK.port}`,
          ],
          chainName: 'Hack Mac',
        },
      ],
...
window.addEventListener('load', async() => {
  ...
  if(window.web3) {
    ...
    window.web3.eth.getAccounts(function (error, accounts) {
      ...
      store.dispatch(actions.loadHackMacContract())
...
function checkWrongNetwork(id) {
  ...
  if(onWrongNetwork) {
    ...
    if(id === constants.NETWORKS.ROPSTEN.id) {
      console.error(`If you want to play on Ropsten, check out https://ropsten.hackmac.openzeppelin.com/`)
    }
  }
