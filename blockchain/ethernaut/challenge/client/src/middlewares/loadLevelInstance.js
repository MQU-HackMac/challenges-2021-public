...
const loadLevelInstance = store => next => action => {
  ...
  if(!instanceAddress && !action.reuse) {
    ...
    // const estimate = await state.contracts.hackmac.getLevelInstance.estimateGas(action.level.deployedAddress)
    ...
    state.contracts.hackmac.createLevelInstance(action.level.deployedAddress, {
