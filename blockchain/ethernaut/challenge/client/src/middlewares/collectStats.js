...
const collectStats = store => next => action => {
  ...
  // console.log(`TRY COLLECT_STATS >`, state.contracts.hackmac !== undefined)
  if(
    !state.network.web3 ||
    !state.contracts.hackmac
  ) {
  ...
  if(!action.createdInstanceLogs) {
    state.contracts.hackmac.LevelInstanceCreatedLog(query.filter, query.range).then(
      (error, result)=> {
  ...
  if(!action.completedLevelLogs) {
    state.contracts.hackmac.LevelCompletedLog(query.filter, query.range).then(
      (error, result)=> {
        if(error) return console.log(error)
